...
public class CompetitionDAO extends AbstractDAO<CompetitionEntity> {
    ...
    public List<Map<String, Object>> findActiveUpcomingCompetitions() throws DAOException {
        List<Map<String, Object>> upcommingGamesList;

        try (PreparedStatement statement = connection.prepareStatement(FIND_ACTIVE_UPCOMING_GAMES_FOR_SETTINGS)) {
            ResultSet resultSet = statement.executeQuery();
            upcommingGamesList = new ArrayList<>();
            ...
            while (resultSet.next()) {
                ...
                upcommingGame.put(SQLFieldConstant.Competition.DATE_FINISH,
                        resultSet.getDate(SQLFieldConstant.Competition.DATE_FINISH));
                upcommingGame.put(SQLFieldConstant.Competition.NAME,
                        resultSet.getString(SQLFieldConstant.Competition.NAME));
                upcommingGamesList.add(upcommingGame);
    ...
    public boolean update(CompetitionEntity entity) throws DAOException {
        ...
        try (PreparedStatement statement = connection.prepareStatement(UPDATE_COMPETITION_COEFFS)) {
