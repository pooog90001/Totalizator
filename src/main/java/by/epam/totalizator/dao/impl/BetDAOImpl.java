...
import static by.epam.totalizator.constant.GeneralConstant.BET;
import static by.epam.totalizator.constant.GeneralConstant.COMPETITION;
...
public class BetDAOImpl extends BetDAO {
    ...
    public List<Map<String, Object>> findPastBetsByUserId(int userId) throws DAOException {
        ...
        try (PreparedStatement statement = connection.prepareStatement(SQLRequestConstant.FIND_PAST_BETS_WITH_GAME_BY_USER_ID)) {
    ...
    public List<Map<String, Object>> findUpcomingBetsByUserId(int userId) throws DAOException {
        ...
        try (PreparedStatement statement = connection.prepareStatement(SQLRequestConstant.FIND_UPCOMING_BETS_BY_USER_ID)) {
            statement.setInt(1, userId);
            ResultSet resultSet = statement.executeQuery();
            betsAndGames = new ArrayList<>();
            while (resultSet.next()) {
    ...
    public boolean create(BetEntity entity) throws DAOException {
        ...
        try (PreparedStatement statement = connection.prepareStatement(SQLRequestConstant.CREATE_BET)) {
    ...
    public void updateCompetitorResultAndPayMoney(int competitionId) throws DAOException {
        try (PreparedStatement statement = connection.prepareStatement(SQLRequestConstant.UPDATE_COMPETITOR_RESULT_AND_PAY_MONEY)) {
            statement.setInt(1, competitionId);
            statement.execute();
    ...
    public void updateGameResultAndPayMoney
        ...
        try (PreparedStatement statement = connection.prepareStatement(SQLRequestConstant.SET_VAR_FIRST_COMPETITOR_RESULT)) {
        ...
        try (PreparedStatement statement = connection.prepareStatement(SQLRequestConstant.SET_VAR_SECOND_COMPETITOR_RESULT)) {
        ...
        try (PreparedStatement statement = connection.prepareStatement(SQLRequestConstant.UPDATE_GAME_RESULT_AND_PAY_MONEY)) {
            statement.setInt(1, competitionId);
            statement.execute();
