...
function edit(e, id) {
    ...
    if ($this.attr('editing') != '1') {
        ...
        $contanier.find('.competitionTypeName').each(function () {
            var value = $(this).text().toString().trim();
            $this.attr('oldName', value);
            var input = $('<input class="competitionTypeName" />').val(value);
        ...
        $contanier.find('.competitionTypeName').each(function () {
            ...
            var div = $('<div class="competitionTypeName" />').text(name);
...
function save(e, id, name) {
    ...
    var dataString = "competitionTypeId=" + id + "&newName="+ name + "&command=update_competition_type";
...
function del(e, id) {
    ...
    //get the form data using another method
    var dataString = "competitionTypeId=" + id + "&command=delete_competition_type";
    //make the AJAX request, dataType is set to json
    //meaning we are expecting JSON data in response from the server
    $.ajax({
        type: "POST",
